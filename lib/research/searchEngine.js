...
class SearchEngine {
  ...
  search(query = '', options = {}) {
    ...
    const scoredDocs = [];
    const allowedIds = Array.isArray(options.documentIds) ? new Set(options.documentIds) : null;
    ...
    // An empty search must not silently reveal the unfiltered corpus.
    const dynamicGraph = { nodes: extractedNodes, edges: extractedEdges };
...
/**
 * Computes Reciprocal Rank Fusion (RRF) scores across multiple ranked lists.
 * Mirrors the hybrid BM25 + kNN rank fusion architecture from the R.A.I.N. DataMatrix Engine (Anna).
 *
 * RRF Score = sum(1 / (k + rank_i))
 *
 * @param {Array<Array<{ id: string, [key: string]: any }>>} rankingsList Array of ranked item lists
...
function reciprocalRankFusion(rankingsList = [], k = 60) {
  ...
  rankingsList.forEach((rankList) => {
    rankList.forEach((item, index) => {
      if (!item || !item.id) return;
      const rank = index + 1;
      const currentScore = scoreMap.get(item.id) || 0;
      scoreMap.set(item.id, currentScore + 1 / (k + rank));
    });
  });
  return scoreMap;
}

SearchEngine.reciprocalRankFusion = reciprocalRankFusion;
SearchEngine.tokenize = tokenize;
SearchEngine.fuzzyTokenMatch = fuzzyTokenMatch;
SearchEngine.scoreField = scoreField;
